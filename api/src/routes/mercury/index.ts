...
// of Vesta on the buy side. Everything here is SECRETS-FREE: MercuryItem/MercuryRequest carry
...
const createItemBody = z.object({
  ...
  vestaSku: z.string().trim().max(64).optional(),
...
const patchItemBody = z.object({
  ...
  vestaSku: z.string().trim().max(64).nullable().optional(),
  active: z.boolean().optional(),
});

const createRequestBody = z.object({
  // Either an existing MercuryItem id, OR a Vesta product ref (sku + displayName) to
...
// Vesta stock (via the shared adjustStock helper); for SECRET items the cloud only records status
...
export async function mercuryRoutes(app: FastifyInstance) {
  ...
  await app.register(async (scoped) => {
    ...
    // GET /api/mercury/items?q= — active items, dash-insensitive search over name + vestaSku.
    scoped.get('/api/mercury/items', async (req) => {
      ...
      const filtered = items.filter(
        (it) =>
          ...
          (it.vestaSku ? flatten(it.vestaSku).includes(needle) : false),
      );
      return { items: filtered };
    });

    // POST /api/mercury/items — create an ordinary item (displayName, optional vestaSku).
    // isSecret is a simple flag for now (full secret resolution is Phase 2). A secret item
    // never stores a vestaSku on the cloud row.
    scoped.post('/api/mercury/items', async (req, reply) => {
      ...
      const vestaSku = secret ? null : (parsed.data.vestaSku || null);
      const item = await prisma.mercuryItem.create({
        data: { displayName, isSecret: secret, vestaSku, active: active ?? true },
      });
      return { ok: true, item };
    });

    // PATCH /api/mercury/items/:id — edit displayName / vestaSku / active.
    scoped.patch<{ Params: { id: string } }>('/api/mercury/items/:id', async (req, reply) => {
      ...
      const data: { displayName?: string; vestaSku?: string | null; active?: boolean } = {};
      if (parsed.data.displayName !== undefined) data.displayName = parsed.data.displayName;
      if (parsed.data.active !== undefined) data.active = parsed.data.active;
      // Never let a secret item carry a vestaSku on the cloud row.
      if (parsed.data.vestaSku !== undefined) {
        data.vestaSku = existing.isSecret ? null : (parsed.data.vestaSku || null);
      }
      const item = await prisma.mercuryItem.update({ where: { id: req.params.id }, data });
      return { ok: true, item };
    });

    // ── Reorder queue (Vesta low-stock feed) ───────────────────────────
    // GET /api/mercury/reorder-queue — the EXACT low-stock query from stock.ts (stock <=
    // reorderPoint). Reuses Vesta's single source of stock truth — no second stock source.
    ...
    scoped.get('/api/mercury/reorder-queue', async (req) => {
      ...
      const [products, items] = await Promise.all([
        ...
        prisma.mercuryItem.findMany({ where: { vestaSku: { in: skus }, active: true } }),
      ]);
      const bySku = new Map(products.map((p) => [p.sku, p]));
      const itemBySku = new Map(items.map((it) => [it.vestaSku!, it]));
    ...
    // (itemId) or resolves/creates one from a Vesta product ref (vestaSku [+ displayName]).
    scoped.post('/api/mercury/requests', async (req, reply) => {
      const parsed = createRequestBody.safeParse(req.body);
      if (!parsed.success) return reply.code(400).send({ error: 'invalid_body' });
      const { itemId, vestaSku, displayName, qty, note } = parsed.data;
      ...
      if (itemId) {
        ...
        // Resolve to (or create) the ordinary MercuryItem for this Vesta SKU.
        const sku = vestaSku.trim();
        ...
        const existing = await prisma.mercuryItem.findFirst({ where: { vestaSku: sku, active: true } });
        if (existing) {
          ...
          const created = await prisma.mercuryItem.create({
            data: { displayName: name, vestaSku: sku, isSecret: false, active: true },
          });
    ...
    // Marks the request 'received' (+ receivedAt) and, for ORDINARY items only, bumps Vesta stock
    // by qty through the SHARED adjustStock helper (the same Product.stock write + StockAdjustment
    // audit row Vesta's /adjust uses — one stock source of truth, Vesta owns it).
    //
    //   ORDINARY item (MercuryItem.vestaSku is set) → status 'received' + adjustStock(+qty).
    //   SECRET item (no vestaSku on the cloud row)   → status 'received' ONLY, NO stock write.
    ...
    scoped.post<{ Params: { id: string } }>('/api/mercury/requests/:id/receive', async (req, reply) => {
      ...
      if (item.isSecret || !item.vestaSku) {
      ...
      // ORDINARY branch: bump Vesta stock by qty through the shared write path, THEN mark received.
