...
function transactionClient() {
  return {
    ...
    // Multi-image link rows (writeMediaLinksInTx inside recordRequestMoneyEventInTx) —
    // these tests don't assert link contents, just that the tx surface exists.
    ceresMediaLink: {
...
vi.mock('../src/db/prisma.js', () => ({
  prisma: {
    ...
    // Route-level array serialization (resolveIdsWithFallback) — no link rows in these tests.
    ceresMediaLink: { findMany: vi.fn(async () => []) },
  },
}));
