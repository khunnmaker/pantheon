...
# Vesta — deploy & ops notes

Vesta is stock management built INTO the Minerva monorepo. It writes `Product.stock`,
`Product.stockAt`, and `Product.reorderPoint` — the same rows Minerva reads — so there is
no sync job. See `docs/VESTA_BRIEF.md` for the spec.

## What was built

- **DB** (`api/prisma/schema.prisma` + migration `20260629000000_vesta_stock`): adds
...
- **Vesta web app** (`vesta/`): a separate Vite/React app + Dockerfile — its own Railway
...
left unchanged** — Vesta never auto-zeros and never auto-creates catalog rows from the
stock file (unmatched SKUs are reported in the preview and skipped).

## Railway

Add ONE new web service for `vesta/` alongside the existing `api` / `web` / Postgres:

1. **New service → Dockerfile** at `vesta/Dockerfile`.
2. Build arg / env **`VITE_API_URL`** = the public URL of the Minerva **api** service
   (baked at build time, same as `web`).
3. The api's **`WEB_ORIGIN`** env must include the Vesta web origin (comma-separated with
   the existing console origin) so CORS allows the browser calls.
4. Vesta shares the same Postgres via the api — it does **not** need its own `DATABASE_URL`
...
Manager logs into Vesta (Dr. M) → **นำเข้า CSV** → upload the Express `.txt` → review the
