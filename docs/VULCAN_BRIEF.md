# Vulcan — Stock Management (build brief)

> Hand this file to a fresh session. It is self-contained: it assumes **no** memory of the
> Minerva chat it was written in. Vulcan is a NEW project that plugs into the existing
> **Minerva** system (a LINE customer-reply assistant for **Prominent**, a Thai dental-equipment
> distributor). Build Vulcan in the **same monorepo** as Minerva.

---

## 1. Mission

Vulcan is a **stock-management tool** for Prominent's product catalog. The warehouse/stock
**manager exports a stock CSV from Express (their Thai accounting program) once a day and
imports it into Vulcan.** Vulcan stores the current quantity per SKU, lets staff adjust it by
hand for corrections, and flags low-stock items. Because Vulcan **shares Minerva's database**,
...
1. **Stock source = daily Express CSV.** The manager exports a stock report from **Express**
   and imports the CSV into Vulcan every day. Express is the upstream master; Vulcan is the
...
3. **Minerva connection = SHARED DATABASE.** Vulcan writes stock into the **same Postgres**
...
- **Repo:** `C:\Users\khunn\Project\Minerva` (monorepo) — `github.com/khunnmaker/minerva`.
  Vulcan lives in THIS repo. Default branch `main` auto-deploys to Railway on push.
...
- **The `Product` model** (`api/prisma/schema.prisma`) — already exists, ~1187 rows, ~1020 with
  stock today (loaded ad-hoc from a periodic stock report — **Vulcan replaces that import path**):
  ```prisma
  model Product {
    ...
    stock     Int?      // remaining qty from latest snapshot (null = unknown)  ← Vulcan writes this
    stockAt   DateTime? // date the stock figure is as-of                       ← Vulcan writes this
    updatedAt DateTime  @updatedAt
  }
  ```
- **SKU scheme:** codes like `01-01-01` / `07-10-09`. This is the shared key between Express,
  Minerva, and Vulcan. Match strictly on `Product.sku`.
- **Minerva ALREADY reads stock.** It shows the exact count + `stockAt` date in the console and
  ...
  **So once Vulcan keeps `Product.stock`/`stockAt` fresh, Minerva needs little-to-no change.**
...
- **One Prisma schema, one database.** Add Vulcan's tables/fields to the EXISTING
  ...
  single migrator.
- **Vulcan writes `Product.stock` + `Product.stockAt`** on import / manual adjust. That IS the
  Minerva integration — Minerva reads those same rows.
- **Backend:** two reasonable options — pick one and note it:
  - (A) Add Vulcan routes to the EXISTING `/api` (e.g. `/api/stock/*`, `/api/stock/import`).
    Least work; shares the Prisma client, auth, and Railway service. **Recommended for v1.**
  - (B) A separate Vulcan api service in the monorepo sharing the same `DATABASE_URL`. Cleaner
    separation, more infra to wire (auth, prisma client, a second migrator to avoid).
- **Frontend:** a dedicated Vulcan UI for the stock manager — either a new Railway web service
  or a separate route-area/app in the monorepo. A separate, simple Vulcan web app is clean; or
  reuse the existing console shell and gate a "สต็อก / Stock" view to the manager. Pick one.
- **Migrations caution:** a Vulcan migration touches Minerva's live DB. Only **ADD** columns/
...
Flow: manager uploads the CSV in Vulcan → **preview** (matched / unmatched / will-change) →
...
## 10. Auth / who uses Vulcan

The **stock manager** (and supervisors) use Vulcan. Reuse Minerva's JWT login. **CONFIRM with
the user:** does the manager use the existing `supervisor` account (Dr. M), or should there be a
separate **manager/stock** role/login? Recommend: reuse the existing login and gate Vulcan to
...
- Railway, same as Minerva. If Vulcan is separate services, point them at the **same**
...
2. **Ask the user for a real sample Express stock CSV** + confirm: columns, encoding, full-snapshot
   semantics (absent SKU = ?), and who logs into Vulcan.
...
2. Who logs into Vulcan (reuse supervisor, or a separate manager role)?
3. Separate Vulcan web/api services vs a stock view inside the existing console — preference?
