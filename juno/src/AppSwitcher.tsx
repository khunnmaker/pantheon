// Grant-aware suite app switcher — small pills next to the "Juno" title that let a logged-in
// user jump to the OTHER suite apps they can access. The current app (juno) shows as a
...
const APPS: { app: AppName; label: string; url: string | undefined }[] = [
  ...
  { app: 'juno', label: 'Juno', url: undefined },
      ...
      {items.map((a) =>
        a.app === CURRENT ? (
          ...
          <a
            ...
            href={a.url}
            title={`ไปที่ ${a.label}`}
            className="px-2 py-0.5 rounded-full text-xs font-medium text-slate-500 hover:text-emerald-700 hover:bg-slate-100"
          >
            {a.label}
          </a>
        ),
