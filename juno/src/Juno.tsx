...
//     · credit:                                 creditUsed > 0      → 'ใช้เครดิตแล้ว' (no new money/recon gate)
...
function stageOf(p: Payment): Stage {
  ...
  if (p.source === 'credit' && Number(p.creditUsed || 0) > 0) {
    return { n: 3, label: 'ใช้เครดิตแล้ว', cls: 'bg-teal-100 text-teal-700' };
  }
...
function MethodCell({ p }: { p: Payment }) {
  if (p.source === 'credit') return <span className="truncate text-emerald-700">เครดิต</span>;
...
function PaymentsView({ view, onChanged, canDelete, isCeo }: { view: Exclude<View, 'reports' | 'recon' | 'reRecon' | 'bills' | 'audit' | 'disc'>; onChanged: () => void; canDelete: boolean; isCeo: boolean }) {
  ...
  const [method, setMethod] = useState<'all' | 'transfer' | 'cash' | 'cheque' | 'credit'>('all');
  ...
  const printablePayment = (payment: Payment) => payment.reNumbers.length + payment.billNos.length > 0;
  const verifiedInView = rows.filter((r) => r.status === 'verified' && printablePayment(r));
  ...
  // Credit-only rows must use the per-row CheckDialog because that is where FIN enters the
  // customer-credit spend; the batch dialog intentionally has no credit controls.
    ...
    <>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3 mb-3 flex flex-wrap items-center gap-2">
          ...
          {view === 'inbox' && (
            <select
              value={method}
              onChange={(e) => setMethod(e.target.value as 'all' | 'transfer' | 'cash' | 'cheque' | 'credit')}
              className="px-2 py-2 rounded-lg border border-slate-300 text-sm bg-white"
              title="วิธีรับเงิน"
            >
          ...
          {view === 'inbox' && (
            <button
              ...
              title={verifiedInView.length === 0
                ? 'ยังไม่มีรายการที่ตรวจแล้ว (มีเลขเอกสาร) ในตัวกรองนี้ — ตรวจรายการก่อนจึงจะพิมพ์ใบปะหน้าได้'
      ...
      {checkedIds.size > 0 && (
        <div className="sticky top-[104px] z-10 bg-emerald-700 text-white rounded-xl px-3 py-2 mb-3 flex flex-wrap items-center gap-2 text-sm">
          ...
          <button
            ...
            title={printableRows.length === 0 ? 'ไม่มีรายการที่เลือกซึ่งมีเลขเอกสาร' : undefined}
...
function AddPaymentModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const valid = method === 'credit'
    ? customerCode.trim() !== '' || customerName.trim() !== ''
    : Number.isFinite(amountNum) && amountNum > 0 && customerName.trim() !== '';
  ...
  async function save() {
    ...
    try {
      await createPayment({
        ...
        amount: method === 'credit' ? '0' : amount.trim(),
        note: note.trim() || undefined,
        ...(method === 'credit'
          ? { receiptName: receiptName.trim() || undefined, transferAt: creditDate }
      ...
      setErr((e as Error).message === 'unauthorized' ? 'เซสชันหมดอายุ — กรุณาเข้าสู่ระบบใหม่' : apiErrorMessage(e));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-4 w-full max-w-md space-y-3 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
          ...
          <>
            <div className="grid grid-cols-2 gap-2">
              ...
              <label className="block">
                ...
                <input value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="กรอกรหัสหรือชื่ออย่างน้อยหนึ่งอย่าง" className={input} />
              </label>
            </div>
            <label className="block">
              <span className={label}>ชื่อบนใบเสร็จ <span className="text-slate-400">— ไม่บังคับ</span></span>
              <input value={receiptName} onChange={(e) => setReceiptName(e.target.value)} placeholder="ชื่อที่จะออกใบเสร็จ" className={input} />
            </label>
            <label className="block">
              <span className={label}>วันที่</span>
              <input type="date" value={creditDate} onChange={(e) => setCreditDate(e.target.value)} className={input} />
            </label>
            <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-2 text-xs text-emerald-800">
              รายการนี้ไม่มีเงินเข้าใหม่ · FIN จะใส่ยอดใช้เครดิตตอนตรวจเอกสาร
            </div>
          </>
        ) : method === 'manual_transfer' ? (
          <>
        ...
        {/* Money-receipt methods keep the tax-invoice request field; credit-only uses receiptName. */}
        {method !== 'credit' && <label className="block">
...
function EditPaymentModal({ payment, onClose, onSaved }: {
  ...
  const amountNum = parseFloat(amount.replace(/[^\d.-]/g, ''));
  const amountValid = payment.source === 'credit'
    ? Number.isFinite(amountNum) && amountNum === 0
    : Number.isFinite(amountNum) && amountNum > 0;
  const valid = payment.source === 'credit'
    ? amountValid && (customerCode.trim() !== '' || customerName.trim() !== '')
    : amountValid && customerName.trim() !== '';
  ...
  async function save() {
    ...
    try {
      const patch: EditPaymentBody = {
        ...
        ...(payment.source === 'credit' ? {} : { amount: amount.trim() }),
        bank: bank.trim(),
...
function Detail({ payment, onClose, onUpdate, onDelete, onPrint, canDelete, isCeo }: {
  ...
  const needsPhysicalReceive = payment.source === 'cash' || payment.source === 'cheque';
  const showExpressConfirm = needsPhysicalReceive || payment.source === 'credit';
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              {showExpressConfirm && rail(
                'recorded',
                p.status !== 'recorded' && p.status !== 'void' && needsPhysicalReceive && !p.receivedAt
                  ? 'ต้องให้ CEO ยืนยันรับเงินก่อนจึงจะยืนยันใน Express ได้'
                  : p.source === 'credit' && Number(p.creditUsed || 0) <= 0
                    ? 'ต้องบันทึกยอดใช้เครดิตก่อนจึงจะยืนยันใน Express ได้'
                  : STATUS_META.recorded.label,
                <CheckCheck size={16} />,
                () => run('recorded', () => setStatus(p.id, 'recorded')),
                {
                  disabled: p.status === 'recorded' || p.status === 'void' ||
                    (needsPhysicalReceive && !p.receivedAt) || (p.source === 'credit' && Number(p.creditUsed || 0) <= 0),
                  active: p.status === 'recorded',
                },
              )}
              {rail('print', p.reNumbers.length + p.billNos.length > 0 ? 'พิมพ์ใบปะหน้า' : 'พิมพ์ใบปะหน้า (ต้องมีเลขเอกสารก่อน)', <Printer size={16} />, () => onPrint(p), {
                disabled: p.reNumbers.length + p.billNos.length === 0,
              })}
        ...
        <div className="flex flex-col xl:flex-row xl:gap-2">
          ...
          <div className="xl:flex-1 xl:min-w-0 xl:border-l xl:border-slate-100">
            ...
            <div className="px-4 grid grid-cols-2 gap-3">
              {field('ช่องทาง', p.source === 'credit' ? 'เครดิต' : p.source === 'cash' ? 'เงินสด' : p.source === 'cheque' ? 'เช็คธนาคาร' : 'ธนาคาร')}
...
function useReceiptChipsInput(initialRe: string[], initialBills: string[], initialWrongTransfer = false) {
  ...
  const [wrongTransfer, setWrongTransfer] = useState(initialWrongTransfer);
  ...
  function addToken(token: ReceiptToken) {
    setMixedError(false);
    if (token.kind === 'wrong_transfer') {
      if (reNumbers.length > 0 || billNos.length > 0) { setMixedError(true); return; }
      setWrongTransfer(true);
      return;
    }
    if (wrongTransfer) { setMixedError(true); return; }
    if (token.kind === 're') {
  ...
  function removeToken(token: ReceiptToken) {
    setMixedError(false);
    if (token.kind === 'wrong_transfer') setWrongTransfer(false);
    else if (token.kind === 're') setReNumbers((prev) => prev.filter((value) => value !== token.value));
  ...
  const pendingMixed = !!pendingToken && (
    pendingToken.kind === 'wrong_transfer'
      ? reNumbers.length > 0 || billNos.length > 0
      : wrongTransfer
  );
  const valid = !pendingMixed && (wrongTransfer || reNumbers.length > 0 || billNos.length > 0 || pendingValid);

  function finalize(): { reNumbers: string[]; billNos: string[]; wrongTransfer: boolean } {
    if (wrongTransfer) return { reNumbers: [], billNos: [], wrongTransfer: true };
    if (!pendingToken) return { reNumbers, billNos, wrongTransfer: false };
    if (pendingToken.kind === 'wrong_transfer') {
      return reNumbers.length || billNos.length
        ? { reNumbers, billNos, wrongTransfer: false }
        : { reNumbers: [], billNos: [], wrongTransfer: true };
    }
    return pendingToken.kind === 're'
      ? { reNumbers: reNumbers.includes(pendingToken.value) ? reNumbers : [...reNumbers, pendingToken.value], billNos, wrongTransfer: false }
      : { reNumbers, billNos: billNos.includes(pendingToken.value) ? billNos : [...billNos, pendingToken.value], wrongTransfer: false };
  }

  function reset(nextRe: string[], nextBills: string[], nextWrongTransfer = false) {
    setReNumbers(nextRe);
    setBillNos(normalizeBillList(nextBills));
    setWrongTransfer(nextWrongTransfer);
    setMixedError(false);
    setReInput('');
  }
  ...
  return {
    reNumbers, billNos, wrongTransfer, mixedError, reInput, addToken, removeToken, onReInputChange,
    pendingToken, pendingValid, pendingInvalid, pendingMixed, valid, finalize, reset, unknownBills, checkingBills,
  };
}
  ...
  return (
    <>
      <div className="mt-0.5 flex flex-wrap gap-1.5 px-2 py-1.5 rounded-lg border border-slate-300 focus-within:ring-2 focus-within:ring-emerald-400">
        {state.wrongTransfer && (
          <span className="flex items-center gap-1 rounded-full bg-rose-600 py-0.5 pl-2 pr-1 text-xs font-semibold text-white">
            โอนเงินผิด 0000000
            <button type="button" onClick={() => state.removeToken({ kind: 'wrong_transfer', value: '0000000' })} className="rounded-full p-0.5 hover:bg-rose-700" title="ยกเลิกการจัดประเภท"><X size={11} /></button>
          </span>
        )}
...
const VERIFY_ERROR_MESSAGES: Record<string, string> = {
  ...
  wrong_transfer_mixed: '0000000 ใช้ร่วมกับเลขเอกสารไม่ได้',
  wrong_transfer_resolution_locked: 'รายการนี้บันทึกการโอนคืนแล้ว — ต้องยืนยันการล้างประวัติก่อน',
  void_locked: 'รายการถูกยกเลิกอยู่ — คืนค่ารายการก่อนแก้ไข',
...
function CheckDialog({ payment, onClose, onSaved }: {
  ...
  async function save() {
    ...
    const undoConfirmed = resettingWrongTransfer && hasRefundAudit
      ? window.confirm('รายการนี้มีประวัติการโอนคืน ต้องการล้างการจัดประเภทและประวัติ แล้วกลับไปรอตรวจใช่หรือไม่?')
      : false;
    if (resettingWrongTransfer && hasRefundAudit && !undoConfirmed) return;
    setSaving(true);
    setErr('');
    try {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-4 w-full max-w-sm space-y-3" onClick={(e) => e.stopPropagation()}>
        ...
        {!re.wrongTransfer && payment.source !== 'credit' && <WhtSection wht={wht} />}

        {!re.wrongTransfer && (payment.source === 'credit' || (creditAvailable ?? 0) > 0 || creditPreview > 0) && <label className="block rounded-lg border border-emerald-200 bg-emerald-50/40 p-2.5">
...
function BatchCheckDialog({ payments, onDone }: {
  ...
  const re = useReceiptChipsInput(current.reNumbers, current.billNos, current.wrongTransfer);
  const [receiptName, setReceiptName] = useState(
  ...
  function goTo(nextIndex: number, nextPayment: Payment) {
    setIndex(nextIndex);
    re.reset(nextPayment.reNumbers, nextPayment.billNos, nextPayment.wrongTransfer);
  ...
  async function saveAndNext() {
    ...
    const documents = re.finalize();
    if (!documents.wrongTransfer && documents.reNumbers.length === 0 && documents.billNos.length === 0) return;
    setSaving(true);
    setErr('');
    try {
  ...
  async function applyToRest() {
    if (!re.valid || re.wrongTransfer || saving) return;
    const documents = re.finalize();
    if (documents.wrongTransfer) return;
        ...
        <div className="flex flex-wrap justify-end gap-2 pt-1">
          ...
          {payments.length - index > 1 && (
            <button
              ...
              disabled={!re.valid || re.wrongTransfer || re.pendingToken?.kind === 'wrong_transfer' || saving}
