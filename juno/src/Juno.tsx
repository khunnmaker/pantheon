...
function PaymentsView({ view, onChanged, canDelete, isCeo }: { view: Exclude<View, 'reports' | 'recon' | 'reRecon' | 'bills' | 'audit' | 'disc'>; onChanged: () => void; canDelete: boolean; isCeo: boolean }) {
  ...
  const checkableRows = checkedRows.filter((r) => r.status !== 'void' && r.status !== 'recorded' && !r.wrongTransfer);
      ...
      <div className="bg-white rounded-2xl p-4 w-full max-w-sm space-y-3" onClick={(e) => e.stopPropagation()}>
        ...
        <label className="block">
          ...
          {!re.wrongTransfer && !re.pendingInvalid && re.reNumbers.length === 0 && re.billNos.length === 0 && !re.pendingValid && (
            <span className="text-[11px] text-slate-400">ต้องมีเลขเอกสาร หรือพิมพ์ 0000000 สำหรับโอนเงินผิด</span>
          )}
        </label>
