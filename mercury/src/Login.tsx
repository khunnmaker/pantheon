...
import { ShoppingCart, LogIn, Loader2, AlertTriangle, ArrowLeft } from 'lucide-react';
...
export default function Login({ onLogin }: { onLogin: (agent: Agent) => void }) {
  ...
  async function submit(useEmail: string, value: string) {
    ...
    try {
      ...
      if (!hasAppAccess(agent, 'mercury')) {
        setError('บัญชีนี้ไม่มีสิทธิ์ใช้งานระบบจัดซื้อ');
        setSecret('');
        setPassword('');
        setBusy(false);
        return;
    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 font-sans text-slate-800">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 max-w-md w-full p-6">
        <div className="flex items-center gap-2 text-orange-700 mb-1">
          ...
          <h1 className="text-xl font-bold">Mercury</h1>
        </div>
        <p className="text-sm text-slate-500 mb-5">จัดซื้อ · สั่งของ · เลือกชื่อเพื่อเข้าสู่ระบบ</p>
        ...
        {view === 'list' ? (
                ...
                {selected.kind === 'password' ? (
                  <>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 mb-3 rounded-md border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
                    />
                    <button
                      onClick={() => submit(selected.email, secret)}
                      disabled={busy || !secret}
                      className="w-full px-3 py-2.5 rounded-md bg-orange-600 hover:bg-orange-700 text-white text-sm font-semibold flex items-center justify-center gap-1 disabled:opacity-50"
                    >
                  ...
                  <>
                    ...
                    <input
                      ...
                      placeholder="••••••"
                      className="w-full px-3 py-2 mb-1 rounded-md border border-slate-300 text-center tracking-[0.5em] text-lg focus:outline-none focus:ring-2 focus:ring-orange-400"
                    />
          ...
          <>
            ...
            <input
              ...
              className="w-full px-3 py-2 mb-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
            />
            <label className="block text-xs font-semibold text-slate-500 mb-1">รหัสผ่าน</label>
            <input
              type="password"
            ...
            <button
              ...
              disabled={busy}
              className="w-full px-3 py-2 rounded-xl bg-orange-600 hover:bg-orange-700 text-white text-sm font-semibold flex items-center justify-center gap-1 disabled:opacity-50"
            >
              {busy ? <Loader2 size={16} className="animate-spin" /> : <LogIn size={16} />} เข้าสู่ระบบ
            </button>
            {!loadFailed && (
        ...
        <p className="text-[10px] text-slate-300 mt-4">Mercury เปิดให้เฉพาะผู้ที่ได้รับสิทธิ์จัดซื้อเท่านั้น</p>
      </div>
    </div>
  );
}
...
function BackButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1 px-2 py-2 -ml-2 mb-3 rounded-md text-sm font-medium text-slate-500 hover:text-orange-600 hover:bg-slate-100"
    >
