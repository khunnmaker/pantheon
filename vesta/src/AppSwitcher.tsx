...
export default function AppSwitcher({ agent }: { agent: Agent }) {
  ...
  // the Boxes icon + "Vesta" text, no chevron, non-interactive.
  if (items.length <= 1) {
    return (
  ...
  return (
    <div className="relative" ref={containerRef} aria-label="สลับแอป">
      <button
        ...
        aria-haspopup="menu"
        aria-expanded={open}
        className="flex items-center gap-2 text-indigo-700 font-bold hover:opacity-80"
      >
        <Boxes size={22} />
        Vesta
        <ChevronDown size={14} />
          ...
          {others.map((a) => {
            ...
            return (
              <a
                ...
                role="menuitem"
                title={`ไปที่ ${a.label}`}
                className="flex items-center gap-2 px-3 py-2 text-sm text-slate-600 hover:bg-indigo-50 hover:text-indigo-700"
              >
                <Icon size={16} />
                {a.label}
              </a>
