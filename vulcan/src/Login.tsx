...
import { Boxes, LogIn, Loader2, AlertTriangle, UserCircle2 } from 'lucide-react';
...
// pre-selected and the credential field is focused. Vulcan is supervisor-only, so the
// ONLY person shown is Dr. M — agents would just be rejected. The supervisor always
// uses a password (never a PIN); the manual mode is a fallback for future roles.
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 font-sans text-slate-800">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 max-w-sm w-full p-6">
        ...
        {mode === 'person' ? (
          <>
            <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-200 rounded-xl px-3 py-2.5 mb-3">
              <UserCircle2 size={22} className="text-indigo-600 shrink-0" />
              <div>
                <div className="text-sm font-semibold text-slate-800">{PERSON.label}</div>
                <div className="text-[11px] text-indigo-600">{PERSON.role}</div>
              </div>
            </div>
