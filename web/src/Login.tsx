    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6 font-sans text-slate-800">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 max-w-sm w-full p-6">
        ...
        <input
          ...
          className="w-full px-3 py-2 mb-3 rounded-xl border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400"
        />
        ...
        <button
          onClick={() => submit()}
          disabled={busy}
          className="w-full px-3 py-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-sm font-semibold flex items-center justify-center gap-1 disabled:opacity-50"
        >
          ...
          <div className="flex flex-wrap gap-1">
            {QUICK.map((q) => (
              <button
                ...
                disabled={busy}
                className="text-xs px-2 py-1 rounded-lg bg-slate-100 hover:bg-teal-100 text-slate-600 border border-slate-200 disabled:opacity-50"
              >
                {q.label}
                {q.role === 'supervisor' && <span className="text-teal-600"> · หัวหน้า</span>}
              </button>
            ))}
          </div>
        </div>
